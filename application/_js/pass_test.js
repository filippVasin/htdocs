    ...
    $.ajax({
        ...
        success: function (answer) {
            ...
            if (request_result == 'ok') {
                ...
                if ($(window).width() < 570){
                    //var name = $("#inst_name_doc").html();
                    //alert(name);
                    var name = $("#inst_name_doc");
                    name.html(name.html().replace(/_/ig, " "));
                }
                if ($(window).width() < 350){
                    $(".navbar-custom-menu a").css("padding-right",0);
                    var name = $("#inst_name_doc").html();
                    var limit_str_position = 30;
                    if (name.length > limit_str_position) {
                        name = name.slice(0, limit_str_position);
                        name = name + "...";
                    }
                    $("#inst_name_doc").html(name);
                }
